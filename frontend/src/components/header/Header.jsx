...
const Header = (props) => {
  ...
  const { loading, error, data, updateData } = useFetch(
    "http://localhost:1337/api/carts?filters[user][username][$eq]=Ivan Golikov&populate[games][populate][0]=photo"
  );
  ...
  const buyGames = async (data) => {
    try {
      const response = await fetch(
        "http://localhost:1337/api/carts/1?populate=*",
        {
          headers,
          method: "GET",
          body: null,
      ...
      if (response.ok) {
        ...
        let ids = [];
        res.data.attributes.games.data.map((game) => ids.push(game.id));
      ...
      await fetch("http://localhost:1337/api/carts/1", {
        ...
        body: JSON.stringify({
          data: {
            ...
            totalprice:
              parseInt(dataR.data.attributes.totalprice) -
  ...
  const Cart = (props) => {
    ...
    return (
      <div className={styles.row}>
        <div>
          {attr.photo.data ? (
            <img
              ...
              src={
                "http://localhost:1337" +
                attr.photo.data.attributes.formats.thumbnail.url
              }
            />
          ) : (
        ...
        <div className={styles.txt}>{attr.title}</div>

        <div className={styles.txt}>{attr.price} CZK</div>
              ...
              <div>
                ...
                {showcart && (
                  <div className={styles.cartInner}>
                    <div className={styles.carthead}>Your Cart:</div>
                    ...
                    <div className={styles.totalPrice}>
                      ...
                      <div className={styles.days}>
                        <button
                          id="lt"
                          value={data.data[0].attributes.days}
                      ...
                      days:{" "}
                      {data.data[0].attributes.totalprice *
                        data.data[0].attributes.days}{" "}
                      CZK
                      <button onClick={buyGames} className={styles.buyBtn}>
                        Buy
                      </button>
                    </div>
                  </div>
